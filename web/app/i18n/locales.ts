// Lightweight i18n dictionary (zh/en). Kept dependency-free on purpose:
// the site has few strings and a full i18n module would be overkill.
// Add a locale by extending `Dict` and `locales` below.

export type Locale = 'zh' | 'en' | 'jp'

export const locales: Locale[] = ['en', 'zh', 'jp']

export const localeLabels: Record<Locale, string> = {
  zh: '中文',
  en: 'English',
  jp: '日本語',
}

type Dict = Record<string, string>

const zh: Dict = {
  'app.title': 'Lolicount',
  'app.desc': '萌系可换肤 SVG 访问计数器,往 README 贴一行链接即可计数。',

  'nav.lang': '语言',
  'nav.top': '顶部',
  'nav.theme': '主题颜色',
  'nav.features': '功能',
  'nav.howto': '使用方法',
  'nav.themes': '主题展示',
  'nav.playground': 'Playground',
  'nav.more': '关于项目',
  'nav.editor': '编辑工作台',

  'emote.currentMotion': '当前动作',
  'emote.motions': '可用动作数',
  'emote.loadFailed': '加载失败',

  'param.animated': '动图',
  'editor.title': '图片编辑工作台',
  'editor.themeName': '主题名称',
  'editor.namePlaceholder': '输入主题名 (字母/数字/连字符)',
  'editor.canvasW': '画布宽度',
  'editor.canvasH': '画布高度',
  'editor.displaySize': '显示尺寸',
  'editor.canvasSettings': '画布设置',
  'editor.layers': '图层',
  'editor.layer': '图层',
  'editor.addLayer': '添加图层',
  'editor.textLayer': '文字层',
  'editor.counterText': '计数文字',
  'editor.fontSize': '字号',
  'editor.scale': '缩放',
  'editor.proportionalScale': '等比缩放',
  'editor.hideText': '隐藏文字',
  'editor.preview': '预览',
  'editor.export': '导出压缩包',
  'editor.modeWorkbench': '工作台',
  'editor.modeQuick': '快速',
  'editor.quickUpload': '快速上传',
  'editor.quickDropHint': '拖放图片到此处，或点击上传',
  'editor.quickLayer': '卡片图层',
  'editor.uploadTheme': '上传主题',
  'editor.uploadGuide': '上传主题指南',
  'editor.uploadStep1Title': 'Fork 仓库',
  'editor.uploadStep1Desc': '在 GitHub 上 Fork Lolicount 仓库到你的账号下。',
  'editor.uploadForkLink': '点击这里 Fork',
  'editor.uploadStep2Title': '克隆到本地',
  'editor.uploadStep2Desc': '将你 Fork 的仓库克隆到本地电脑。',
  'editor.uploadStep3Title': '导出主题',
  'editor.uploadStep3Desc': '在编辑器中完成主题后，点击导出压缩包下载 zip 文件。',
  'editor.uploadStep4Title': '放入主题目录',
  'editor.uploadStep4Desc': '解压 zip 文件，将主题文件夹放入 assets/theme/ 目录。',
  'editor.uploadStep4Note': '卡片主题为 0.webp, 1.webp...；立绘主题为 ren.json + config.json + ren/ 目录。',
  'editor.uploadStep5Title': '提交代码',
  'editor.uploadStep5Desc': '创建新分支，添加主题文件并提交。',
  'editor.uploadStep6Title': '创建 Pull Request',
  'editor.uploadStep6Desc': '在 GitHub 上向 Lolicount 主仓库发起 Pull Request。',
  'editor.uploadPrLink': '创建 Pull Request',
  'editor.uploadStep7Title': '等待审核',
  'editor.uploadStep7Desc': '维护者会审核你的主题，通过后合并并自动重建前端。',
  'editor.uploadEmailHint': '或者将压缩包发到邮箱：',
  'editor.exportImage': '导出图片',
  'editor.previewHint': '点击预览查看渲染效果',
  'editor.cardTheme': '卡片主题 (单图层)',
  'editor.characterTheme': '立绘主题 (多图层)',
  'editor.errNoLayers': '至少需要一个图层',
  'editor.errNoName': '请输入主题名称',
  'editor.layerZ': '层级',
  'editor.imgUnit': '图',
  'editor.uploadImage': '上传图片',
  'editor.noImages': '暂无图片',
  'editor.selectImage': '选择显示',
  'editor.savedDrafts': '已保存草稿',
  'editor.saveDraft': '保存草稿',
  'editor.draftNamePrompt': '输入草稿名称',
  'editor.showCanvas': '显示画布与坐标',

  'hero.title': 'Lolicount',

  'howto.title': 'How to use',
  'howto.introPre': '进入下方的',
  'howto.introLink': 'Playground',
  'howto.introPost': ',填入你的名字,点击生成,再将生成器下方的链接贴到你想要的地方即可。',
  'howto.mdHint': '如果是 Markdown 编辑器,推荐使用:',
  'howto.example': '示例',

  'loli.loading': '加载中…',

  'themes.title': '主题展示',
  'themes.desc': '所有主题统一展示。单图层主题(卡片)和多图层主题(立绘)均可选择,点击图片可重新加载',
  'themes.reload': '重新加载',
  'themes.select': '选择主题',
  'themes.reloadHint': '点击右侧图片可重新加载',
  'themes.variants': '该主题的变化数：{n}',

  'themesGallery.title': '主题库',
  'themesGallery.desc': '按游戏作品、角色类型浏览全部主题,支持罗马音、日文名或主题名搜索。点击卡片选中主题,右侧预览并生成计数器。',
  'themesGallery.allGames': '全部游戏',
  'themesGallery.allKinds': '全部类型',
  'themesGallery.kindCard': '单图层',
  'themesGallery.kindCharacter': '多图层',
  'themesGallery.searchPlaceholder': '搜索主题 / 角色 / 罗马音 / 游戏',
  'themesGallery.resultCount': '共 {n} 个主题',
  'themesGallery.noResults': '没有匹配的主题,换个关键词试试',
  'themesGallery.previewTitle': '主题预览',
  'themesGallery.previewHint': '点击图片可重新加载(随机换一帧)',
  'themesGallery.gameLabel': '来源游戏',
  'themesGallery.characterLabel': '角色',
  'themesGallery.unknownMeta': '暂无角色信息',
  'themesGallery.quickStart': '快速开始',
  'themesGallery.quickStartDesc': '输入名称,选择主题,一键生成。需要更多角色?去主题库挑选。',
  'themesGallery.browseThemes': '浏览主题库',


  'playground.title': 'Playground',
  'playground.params': '参数',
  'playground.preview': '预览',
  'playground.emptyHint1': '输入名字点击生成',
  'playground.emptyHint2': '在下方获取链接',
  'playground.generate': '生成!',
  'playground.regenerate': '重新生成',
  'playground.modeQuick': '快速模式',
  'playground.modeFine': '微调模式',
  'playground.modeExpert': '专家模式',
  'about.title': '关于项目',
  'common.expand': '展开',
  'common.collapse': '折叠',
  'about.desc': 'Lolicount 是一个萌系可换肤的 SVG 访问计数器,往 README 贴一行链接即可开始计数。支持卡片主题与立绘主题,可自定义字体、位置、缩放等参数。',
  'notice.title': '主题动态展示',
  'notice.desc': '所有主题实时渲染,自动翻页,鼠标悬停暂停。',
  'notice.prev': '上一张',
  'notice.next': '下一张',

  'embed.title': '嵌入方式',
  'embed.svg': 'SVG 地址',
  'embed.img': 'Img 标签',
  'embed.markdown': 'Markdown',
  'embed.widget': '挂件 HTML',
  'embed.copy': '复制',
  'embed.copied': '已复制',

  'playground.animatedHint': '动图主题通过挂件脚本嵌入，需要支持 HTML 与 WebGL 的页面（GitHub README 无法运行脚本，请使用普通主题的图片方式）。',

  'param.name': '计数器名称',
  'param.namePlaceholder': '请输入计数器名称',
  'param.nameEmpty': '请先输入计数器名称',
  'param.theme': '主题',
  'param.fontStyle': '字体样式',
  'param.text': '文字模板 text',
  'param.textPlaceholder': '用 {n} 表示计数数字，如 views:{n}',
  'param.fontDefault': '默认',
  'param.fsize': '字号 fsize',
  'param.scale': '图片缩放 scale',
  'param.px': '像素 x',
  'param.py': '像素 y',
  'param.rx': '比例 rx',
  'param.ry': '比例 ry',
  'param.unshowf': '隐藏字体 (unshowf)',
  'param.number': '固定展示数字 number,0 为关闭',
  'param.numberHint': '大于 0 时直接展示该值,不计数也不 +1。',

  'tool.param': '参数 Param',
  'tool.description': '说明 Description',
  'tool.value': '值 Value',
  'tool.unusual': '非常规选项 Unusual Options',

  'footer.credits': '感谢所有贡献者',
  'footer.poweredBy': '由 ❤ 与 Go + Nuxt 驱动',
 'footer.repo': 'GitHub 仓库',
 'footer.author': '作者',
 'footer.donate': '赞助本项目',
  'footer.starHint': '真的真的很需要你的一个 star 作为我更新的动力！',
  'footer.donateHint': '如果 Lolicount 对你有帮助,可以请作者喝杯奶茶 🧋',
  'footer.contributors': '贡献者',
  'footer.tagline': '萌系可换肤 SVG 访问计数器',
  'footer.author': '作者',
  'footer.repo': 'GitHub 仓库',
  'footer.thanks': '致谢项目',
}

const en: Dict = {
  'app.title': 'Lolicount',
  'app.desc': 'A cute, themeable SVG visitor counter — paste one link in your README and watch it count.',

  'nav.lang': 'Language',
  'nav.top': 'Top',
  'nav.theme': 'Theme color',
  'nav.features': 'Features',
  'nav.howto': 'How to use',
  'nav.themes': 'Cards',
  'nav.playground': 'Playground',
  'nav.more': 'About',
  'nav.editor': 'Editor',

  'emote.currentMotion': 'Current motion',
  'emote.motions': 'Motions',
  'emote.loadFailed': 'Load failed',

  'param.animated': 'animated',
  'editor.title': 'Theme Editor',
  'editor.themeName': 'Theme Name',
  'editor.namePlaceholder': 'Enter name (letters/digits/hyphens)',
  'editor.canvasW': 'Canvas Width',
  'editor.canvasH': 'Canvas Height',
  'editor.displaySize': 'Display Size',
  'editor.canvasSettings': 'Canvas Settings',
  'editor.layers': 'Layers',
  'editor.layer': 'Layer',
  'editor.addLayer': 'Add Layer',
  'editor.textLayer': 'Text Layer',
  'editor.counterText': 'Counter Text',
  'editor.fontSize': 'Font Size',
  'editor.scale': 'Scale',
  'editor.proportionalScale': 'Proportional Scale',
  'editor.hideText': 'Hide Text',
  'editor.preview': 'Preview',
  'editor.export': 'Export ZIP',
  'editor.modeWorkbench': 'Workbench',
  'editor.modeQuick': 'Quick',
  'editor.quickUpload': 'Quick Upload',
  'editor.quickDropHint': 'Drag and drop images here, or click to upload',
  'editor.quickLayer': 'Card Layer',
  'editor.uploadTheme': 'Upload Theme',
  'editor.uploadGuide': 'Upload Theme Guide',
  'editor.uploadStep1Title': 'Fork the Repository',
  'editor.uploadStep1Desc': 'Fork the Lolicount repository on GitHub to your account.',
  'editor.uploadForkLink': 'Click here to Fork',
  'editor.uploadStep2Title': 'Clone Locally',
  'editor.uploadStep2Desc': 'Clone your forked repository to your local machine.',
  'editor.uploadStep3Title': 'Export Theme',
  'editor.uploadStep3Desc': 'Finish your theme in the editor, then click Export ZIP to download.',
  'editor.uploadStep4Title': 'Place in Theme Directory',
  'editor.uploadStep4Desc': 'Unzip the file and place the theme folder into assets/theme/.',
  'editor.uploadStep4Note': 'Card themes: 0.webp, 1.webp...; Character themes: ren.json + config.json + ren/ directory.',
  'editor.uploadStep5Title': 'Commit Changes',
  'editor.uploadStep5Desc': 'Create a new branch, add the theme files, and commit.',
  'editor.uploadStep6Title': 'Create Pull Request',
  'editor.uploadStep6Desc': 'Open a Pull Request against the main Lolicount repository on GitHub.',
  'editor.uploadPrLink': 'Create Pull Request',
  'editor.uploadStep7Title': 'Wait for Review',
  'editor.uploadStep7Desc': 'Maintainers will review your theme. Once merged, the frontend rebuilds automatically.',
  'editor.uploadEmailHint': 'Or send the ZIP to:',
  'editor.exportImage': 'Export Image',
  'editor.previewHint': 'Click Preview to render',
  'editor.cardTheme': 'Card theme (single layer)',
  'editor.characterTheme': 'Character theme (multi-layer)',
  'editor.errNoLayers': 'At least one layer is required',
  'editor.errNoName': 'Please enter a theme name',
  'editor.layerZ': 'Z',
  'editor.imgUnit': 'img',
  'editor.uploadImage': 'Upload Image',
  'editor.noImages': 'No images',
  'editor.selectImage': 'Select to display',
  'editor.savedDrafts': 'Saved Drafts',
  'editor.saveDraft': 'Save Draft',
  'editor.draftNamePrompt': 'Enter draft name',
  'editor.showCanvas': 'Show Canvas & Grid',

  'hero.title': 'Lolicount',

  'howto.title': 'How to use',
  'howto.introPre': 'Scroll down to the ',
  'howto.introLink': 'Playground',
  'howto.introPost': ', enter your name, click Generate, then paste the link below the generator wherever you like.',
  'howto.mdHint': 'For a Markdown editor, the recommended format is:',
  'howto.example': 'e.g.',

  'loli.loading': 'Loading…',

  'themes.title': 'Card Themes',
  'themes.desc': 'Each card theme shows a single image. Click the image to reload it.',
  'themes.reload': 'Reload',
  'themes.select': 'Select theme',
  'themes.reloadHint': 'Click the image to reload it',
  'themes.variants': 'Variations: {n}',

  'themesGallery.title': 'Theme Gallery',
  'themesGallery.desc': 'Browse all themes by game and character type. Search by theme name, character, romaji or game. Click a card to select, preview and generate on the right.',
  'themesGallery.allGames': 'All games',
  'themesGallery.allKinds': 'All types',
  'themesGallery.kindCard': 'Card',
  'themesGallery.kindCharacter': 'Character',
  'themesGallery.searchPlaceholder': 'Search theme / character / romaji / game',
  'themesGallery.resultCount': '{n} themes',
  'themesGallery.noResults': 'No themes match — try another keyword',
  'themesGallery.previewTitle': 'Theme Preview',
  'themesGallery.previewHint': 'Click the image to reload (random frame)',
  'themesGallery.gameLabel': 'Game',
  'themesGallery.characterLabel': 'Character',
  'themesGallery.unknownMeta': 'No character info yet',
  'themesGallery.quickStart': 'Quick Start',
  'themesGallery.quickStartDesc': 'Enter a name, pick a theme, generate. Need more characters? Browse the theme gallery.',
  'themesGallery.browseThemes': 'Browse Theme Gallery',


  'playground.title': 'Playground',
  'playground.params': 'Parameters',
  'playground.preview': 'Preview',
  'playground.emptyHint1': 'Enter a name and click Generate',
  'playground.emptyHint2': 'Get the link below',
  'playground.generate': 'Generate it!',
  'playground.regenerate': 'Regenerate',
  'playground.modeQuick': 'Quick',
  'playground.modeFine': 'Fine-tune',
  'playground.modeExpert': 'Expert',
  'about.title': 'About',
  'common.expand': 'Expand',
  'common.collapse': 'Collapse',
  'about.desc': 'Lolicount is a cute, themeable SVG visitor counter — paste one link in your README and it starts counting. Supports card and character themes with customizable fonts, positions, and scaling.',
  'notice.title': 'Live Theme Showcase',
  'notice.desc': 'Every theme rendered live, auto-paging, pause on hover.',
  'notice.prev': 'Previous',
  'notice.next': 'Next',

  'embed.title': 'Embed Formats',
  'embed.svg': 'SVG address',
  'embed.img': 'Img tag',
  'embed.markdown': 'Markdown',
  'embed.widget': 'Widget HTML',
  'embed.copy': 'Copy',
  'embed.copied': 'Copied',

  'playground.animatedHint': 'Animated themes embed via a widget script and need an HTML + WebGL capable page (GitHub READMEs cannot run scripts — keep using the image embed there).',

  'param.name': 'Counter name',
  'param.namePlaceholder': 'Enter your counter name',
  'param.nameEmpty': 'Please enter a counter name first',
  'param.theme': 'Theme',
  'param.fontStyle': 'Font style',
  'param.text': 'Text template',
  'param.textPlaceholder': 'Use {n} for the count, e.g. views:{n}',
  'param.fontDefault': 'Default',
  'param.fsize': 'Font size',
  'param.scale': 'Image scale',
  'param.px': 'Pixel x',
  'param.py': 'Pixel y',
  'param.rx': 'Ratio rx',
  'param.ry': 'Ratio ry',
  'param.unshowf': 'Hide font (unshowf)',
  'param.number': 'Fixed display number, 0 to disable',
  'param.numberHint': 'When > 0 the value is shown as-is without counting or +1.',

  'tool.param': 'Param',
  'tool.description': 'Description',
  'tool.value': 'Value',
  'tool.unusual': 'Unusual Options',

  'footer.credits': 'Thanks to all contributors',
  'footer.poweredBy': 'Powered by ❤ and Go + Nuxt',
 'footer.repo': 'GitHub repo',
 'footer.author': 'Author',
 'footer.donate': 'Sponsor this project',
  'footer.starHint': "I really, really need your star — it's what keeps me updating!",
  'footer.donateHint': 'If Lolicount helps you, buy the author a milk tea 🧋',
  'footer.contributors': 'Contributors',
  'footer.tagline': 'A cute, themeable SVG visitor counter',
  'footer.author': 'Author',
  'footer.repo': 'GitHub repo',
  'footer.thanks': 'Acknowledgements',
}

const jp: Dict = {
  'app.title': 'Lolicount',
  'app.desc': '萌える系スキン変更可能な SVG アクセスカウンター。README にリンクを 1 行貼るだけでカウント開始。',

  'nav.lang': '言語',
  'nav.top': 'トップ',
  'nav.theme': 'テーマカラー',
  'nav.features': '機能',
  'nav.howto': '使い方',
  'nav.themes': 'テーマ',
  'nav.playground': 'Playground',
  'nav.more': '概要',
  'nav.editor': 'エディター',

  'emote.currentMotion': '現在のモーション',
  'emote.motions': 'モーション数',
  'emote.loadFailed': '読み込みに失敗しました',

  'param.animated': 'アニメ',
  'editor.title': 'テーマエディター',
  'editor.themeName': 'テーマ名',
  'editor.namePlaceholder': '名前を入力 (英数字/ハイフン)',
  'editor.canvasW': 'キャンバス幅',
  'editor.canvasH': 'キャンバス高さ',
  'editor.displaySize': '表示サイズ',
  'editor.canvasSettings': 'キャンバス設定',
  'editor.layers': 'レイヤー',
  'editor.layer': 'レイヤー',
  'editor.addLayer': 'レイヤー追加',
  'editor.textLayer': 'テキストレイヤー',
  'editor.counterText': 'カウンターテキスト',
  'editor.fontSize': 'フォントサイズ',
  'editor.scale': 'スケール',
  'editor.proportionalScale': '等比スケール',
  'editor.hideText': 'テキスト非表示',
  'editor.preview': 'プレビュー',
  'editor.export': 'ZIPエクスポート',
  'editor.modeWorkbench': 'ワークベンチ',
  'editor.modeQuick': 'クイック',
  'editor.quickUpload': 'クイックアップロード',
  'editor.quickDropHint': '画像をここにドラッグ＆ドロップ、またはクリックしてアップロード',
  'editor.quickLayer': 'カードレイヤー',
  'editor.uploadTheme': 'テーマアップロード',
  'editor.uploadGuide': 'テーマアップロードガイド',
  'editor.uploadStep1Title': 'リポジトリをフォーク',
  'editor.uploadStep1Desc': 'GitHubでLolicountリポジトリをあなたのアカウントにフォークします。',
  'editor.uploadForkLink': 'ここをクリックしてフォーク',
  'editor.uploadStep2Title': 'ローカルにクローン',
  'editor.uploadStep2Desc': 'フォークしたリポジトリをローカルにクローンします。',
  'editor.uploadStep3Title': 'テーマをエクスポート',
  'editor.uploadStep3Desc': 'エディタでテーマを完成させ、ZIPエクスポートをクリックしてダウンロードします。',
  'editor.uploadStep4Title': 'テーマディレクトリに配置',
  'editor.uploadStep4Desc': 'ZIPを解凍し、テーマフォルダを assets/theme/ に配置します。',
  'editor.uploadStep4Note': 'カードテーマ: 0.webp, 1.webp...; キャラクターテーマ: ren.json + config.json + ren/ ディレクトリ。',
  'editor.uploadStep5Title': '変更をコミット',
  'editor.uploadStep5Desc': '新しいブランチを作成し、テーマファイルを追加してコミットします。',
  'editor.uploadStep6Title': 'Pull Requestを作成',
  'editor.uploadStep6Desc': 'GitHubでLolicountメインリポジトリにPull Requestを作成します。',
  'editor.uploadPrLink': 'Pull Requestを作成',
  'editor.uploadStep7Title': 'レビュー待ち',
  'editor.uploadStep7Desc': 'メンテナーがテーマをレビューします。マージ後、フロントエンドが自動的に再構築されます。',
  'editor.uploadEmailHint': 'またはZIPをメールで送信：',
  'editor.exportImage': '画像エクスポート',
  'editor.previewHint': 'プレビューをクリックしてレンダリング',
  'editor.cardTheme': 'カードテーマ (単一レイヤー)',
  'editor.characterTheme': 'キャラクターテーマ (複数レイヤー)',
  'editor.errNoLayers': 'レイヤーが少なくとも1つ必要です',
  'editor.errNoName': 'テーマ名を入力してください',
  'editor.layerZ': 'Z',
  'editor.imgUnit': '枚',
  'editor.uploadImage': '画像アップロード',
  'editor.noImages': '画像なし',
  'editor.selectImage': '表示選択',
  'editor.savedDrafts': '保存済みドラフト',
  'editor.saveDraft': 'ドラフト保存',
  'editor.draftNamePrompt': 'ドラフト名を入力',
  'editor.showCanvas': 'キャンバスと座標を表示',

  'hero.title': 'Lolicount',

  'howto.title': '使い方',
  'howto.introPre': '下の',
  'howto.introLink': 'Playground',
  'howto.introPost': 'で名前を入力し、生成を押して、下に表示されたリンクを好きな場所に貼るだけです。',
  'howto.mdHint': 'Markdown エディタならこちらがおすすめ:',
  'howto.example': '例',

  'loli.loading': '読み込み中…',

  'themes.title': 'テーマギャラリー',
  'themes.desc': 'すべてのテーマを統合表示。単一レイヤー(カード)と複数レイヤー(キャラクター)の両方を選択できます。画像をクリックで再読み込み。',
  'themes.reload': '再読み込み',
  'themes.select': 'テーマ選択',
  'themes.reloadHint': '右の画像をクリックで再読み込み',
  'themes.variants': 'バリエーション数：{n}',

  'themesGallery.title': 'テーマギャラリー',
  'themesGallery.desc': 'ゲーム・キャラクタータイプ別に全テーマを閲覧。テーマ名・キャラ名・ローマ字・ゲーム名で検索できます。カードをクリックして選択し、右側でプレビューと生成。',
  'themesGallery.allGames': 'すべてのゲーム',
  'themesGallery.allKinds': 'すべてのタイプ',
  'themesGallery.kindCard': 'カード',
  'themesGallery.kindCharacter': 'キャラクター',
  'themesGallery.searchPlaceholder': 'テーマ / キャラ / ローマ字 / ゲームを検索',
  'themesGallery.resultCount': '{n} 件のテーマ',
  'themesGallery.noResults': '一致するテーマがありません。別のキーワードを試してください',
  'themesGallery.previewTitle': 'テーマプレビュー',
  'themesGallery.previewHint': '画像をクリックで再読み込み(ランダムフレーム)',
  'themesGallery.gameLabel': 'ゲーム',
  'themesGallery.characterLabel': 'キャラクター',
  'themesGallery.unknownMeta': 'キャラ情報はまだありません',
  'themesGallery.quickStart': 'クイックスタート',
  'themesGallery.quickStartDesc': '名前を入力してテーマを選び、ワンクリックで生成。もっとキャラを見るならテーマギャラリーへ。',
  'themesGallery.browseThemes': 'テーマギャラリーを見る',


  'playground.title': 'Playground',
  'playground.params': 'パラメータ',
  'playground.preview': 'プレビュー',
  'playground.emptyHint1': '名前を入力して生成を押してください',
  'playground.emptyHint2': '下にリンクが表示されます',
  'playground.generate': '生成!',
  'playground.regenerate': '再生成',
  'playground.modeQuick': 'クイック',
  'playground.modeFine': '微調整',
  'playground.modeExpert': 'エキスパート',
  'about.title': '概要',
  'common.expand': '展開',
  'common.collapse': '折りたたむ',
  'about.desc': 'Lolicount は萌える系スキン変更可能な SVG アクセスカウンターです。README にリンクを 1 行貼るだけでカウント開始。カードテーマとキャラクターテーマをサポートし、フォント、位置、倍率などをカスタマイズできます。',
  'notice.title': 'テーマ動的展示',
  'notice.desc': '全テーマをリアルタイム描画、自動ページ送り、ホバーで一時停止。',
  'notice.prev': '前へ',
  'notice.next': '次へ',

  'embed.title': '埋め込み方法',
  'embed.svg': 'SVG アドレス',
  'embed.img': 'Img タグ',
  'embed.markdown': 'Markdown',
  'embed.widget': 'ウィジェット HTML',
  'embed.copy': 'コピー',
  'embed.copied': 'コピー済み',

  'playground.animatedHint': 'アニメテーマはウィジェットスクリプトで埋め込みます。HTML + WebGL が使えるページが必要です（GitHub README ではスクリプトが実行できないため、引き続き画像埋め込みをご利用ください）。',

  'param.name': 'カウンター名',
  'param.namePlaceholder': 'カウンター名を入力',
  'param.nameEmpty': '先にカウンター名を入力してください',
  'param.theme': 'テーマ',
  'param.fontStyle': 'フォントスタイル',
  'param.text': 'テキストテンプレート',
  'param.textPlaceholder': '{n} でカウント数を表す、例: views:{n}',
  'param.fontDefault': 'デフォルト',
  'param.fsize': 'フォントサイズ fsize',
  'param.scale': '画像倍率 scale',
  'param.px': 'ピクセル x',
  'param.py': 'ピクセル y',
  'param.rx': '比率 rx',
  'param.ry': '比率 ry',
  'param.unshowf': 'フォント非表示 (unshowf)',
  'param.number': '固定表示数字 number、0 で無効',
  'param.numberHint': '0 より大きい値はそのまま表示し、カウントも +1 も行いません。',

  'tool.param': 'パラメータ Param',
  'tool.description': '説明 Description',
  'tool.value': '値 Value',
  'tool.unusual': '特殊オプション Unusual Options',

  'footer.credits': '全コントリビューターに感謝',
  'footer.poweredBy': '❤ と Go + Nuxt で駆動',
  'footer.repo': 'GitHub リポジトリ',
  'footer.author': '作者',
  'footer.donate': 'プロジェクトを支援',
  'footer.starHint': '更新の励みに、ぜひ star をお願いします！',
  'footer.donateHint': 'Lolicount がお役に立てば、作者にミルクティーを奢ってください 🧋',
  'footer.contributors': 'コントリビューター',
  'footer.tagline': '萌える系スキン変更可能な SVG アクセスカウンター',
  'footer.thanks': '謝辞',
}

export const dictionaries: Record<Locale, Dict> = { zh, en, jp }
